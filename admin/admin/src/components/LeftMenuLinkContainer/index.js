...
const teacherContentList = ["exercise", "milestone", "timeline"]

function LeftMenuLinkContainer({ layout, plugins, userInfo }) {
  ...
  const linkSections = Object.keys(pluginsSections).map((current, j) => {
    ...
    let contentTypes = contentTypesToShow
    ...
    if (userInfo && userInfo.type) {
      contentTypes = contentTypes.filter(item => {
        return teacherContentList.find(i => i === item.destination)
      })
    }
